...
public class CreatePDF {
  ...
  public String urlFactura(String name) throws IOException {
    ...
    // SI EXISTE SE LE AGREGA UN VALOR ENTRE PARENTESIS PARA GUARDAR EL NUEVO
    // ARCHIVO
    while ((new File(curDir)).exists()) {
      if (repeat > 1)
        curDir = curDir.substring(0, lastPos) + "(" + Integer.toString(repeat) + ").pdf";
      else
        curDir = curDir.substring(0, lastPos) + "(1).pdf";
      repeat++;
    }
