...
public class FacturaContenido {
  ...
  private final Integer CARACTERES_POR_LINEA = 82; // INDICA EL NUMERO MAXIMO DE CARACTERES POSIBLES EN UNA LINEA DEL
                                                   // PDF.
  ...
  /**
   * Función que permite imprimir la información de los paquetes sin salirse del
   * limite inferior de las paginas del PDF
   * 
  ...
  public void drawTableMultiPage(PDPageContentStream contentStream, float rowHeight) throws IOException {
    ...
    while (texto.length - nextRow > rowsPerPage) { // Split
      ...
      PDFTableGenerator.drawTable(document, contentStream, Arrays.copyOfRange(texto, nextRow, nextRow + rowsPerPage),
          "", XTABLE, yTable, rowHeight);
    ...
    PDFTableGenerator.drawTable(document, contentStream, Arrays.copyOfRange(texto, nextRow, texto.length), monto,
        XTABLE, yTable, rowHeight);
    PDFTableGenerator.drawHeader();
    contentStream.close();
  ...
  public void crearFactura() throws IOException {
    ...
    PDFBillGenerator.drawPayInfo(document, contentStream, Arrays.copyOfRange(infoPago, 1, infoPago.length),
        tableHeight);
    contentStream.close();
