...
public class GeneralChecker {
  ...
  public static boolean checkTextFieldEmptyAndFC(TextField textFields[]) {
    for (TextField textField : textFields) {
      if (!checkTextFieldEmpty(textField))
        return false;
      if (!checkTextFieldForbiddenCharacters(textField))
        return false;
    }
    return true;
  }
   ...
   * @return true si tiene edad superior a los años enviados. false de lo
   *         contrario.
   */
  public static Boolean checkFecha(LocalDate fecha, int años) {
    if (fecha == null)
      return false;
    if (fecha.isAfter(LocalDate.now().minusYears(años)))
      return true;
    return false;
  }
