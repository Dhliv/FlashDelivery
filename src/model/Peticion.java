...
/**
 * Clase Petición. Contiene parámetros relacionados a la petición de recogida.
 * Posee métodos para la comunicación con la BD.
 * 
 * @author David Henao
...
public class Peticion {
  ...
  /**
   * Retorna una lista con las peticiones del empleado con cedula
   * {@code id_empleado} y que esten en estado 'Pendiente', formateadas para ser
   * presentadas en una TableView
   * 
  ...
  public static ObservableList<FormattedData> getPeticionesFormattedData(String id_empleado) {
    /*
     * List<FormattedData> peticiones = Conexion.db().select(DSL.field("id"),
     * DSL.field("descripcion"), DSL.field("nombre"), DSL.field("ciudad"),
     * DSL.field("direccion"),
     * DSL.field("telefono")).from("peticion_recogida").join("cliente")
     * .on(DSL.field("cedula").eq(DSL.field("cliente_solicitud"))).where(DSL.field(
     * "id_empleado").eq(id_empleado)).and(DSL.field("estado").eq(2)).fetch().into(
     * FormattedData.class); Conexion.closeConnection();
     */
    List<FormattedData> peticiones = Conexion.db().fetch(
        "select id, descripcion, nombre, ciudad, direccion, telefono from peticion_recogida join cliente on cedula=cliente_solicitud where id_empleado='"
            + id_empleado + "' and estado='Pendiente'")
        .into(FormattedData.class);
    return FXCollections.observableArrayList(peticiones);
  }

  /**
   * Clase encargada de representar los datos formateados de una Peticion para una
   * TableView
   * 
   * @author Julián Orejuela
   */
